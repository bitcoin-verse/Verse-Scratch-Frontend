...
const web3 = new Web3(
  new Web3.providers.HttpProvider(
    'https://eth-mainnet.g.alchemy.com/v2/jOIyWO860V1Ekgvo9-WGdjDgNr2nYxlh'
  )
)
...
export default {
  ...
  setup () {
    ...
    const products = computed(() =>
      store.getProducts().filter(product => product.active == true)
    )
    ...
    async function onTicketInputChange () {
      ...
      timeoutId = setTimeout(async () => {
        ...
        if (ticketInputAddress.value.length != 42) {
          try {
            const address = await web3.eth.ens.getAddress(
              ticketInputAddress.value
            )
            if (address.length > 0) {
    ...
    async function purchaseTicket (_giftAddress) {
      ...
      try {
        if (buyStep.value == 2) {
          ...
          if (
            verseAllowance.value >=
            activeProduct.value.ticketPrice * validatedAmount.value
          ) {
      ...
      try {
        ...
        if (data) {
          ...
          if (activeProduct.multibuy) {
            ...
            if (
              verseAllowance.value >= activeProduct.value.ticketPrice &&
              buyStep.value == 3
            ) {
              buyStep.value = 4
            }
    ...
    async function getBalance () {
      try {
        ...
        if (data) {
          ...
          if (
            activeProduct.value.multibuy == false &&
            verseBalance.value < activeProduct.value.ticketPrice
          ) {
            buyStep.value = 1
          } else if (
            verseBalance.value >= activeProduct.value.ticketPrice &&
