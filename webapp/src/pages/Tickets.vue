...
export default {
    ...
    setup() {
        ...
        if (getAccount(wagmiConfig).address && getAccount(wagmiConfig).address.length != undefined) {
...
<template>
    <div
        class="backdrop"
        v-if="claimActive || giftModal || newTicketModal"
    >
        ...
        >
            ...
            <div class="modal-body collection">
                ...
                <div class="collection-picker">
                    <div
                        class="collection"
                        v-for="item in products.reverse()"
                        :style="`background-image: ${item.bannerLarge}`"
                    >
                        <h2>{{ item.title.toUpperCase() }}</h2>
                        <div class="overview">
                            <div
                                class="left"
                                :style="`background-color: ${item.jackpotBoxColorOne}`"
                            >
                        ...
                        <a :href="`/?campaign=${item.campaign}&purchase-intent=true`"><button
                                class="btn-select-col"
                                :style="`background-color: ${item.homeLinkColor}`"
                            >Buy From This Collection</button></a>
                    </div>
                </div>
            </div>
        </div>
        ...
        >
            ...
            <div class="modal-body short">
                ...
                <p class="subtext">Somebody has sent a scratch ticket to you. Your ticket has a chance to win
                    <span>1.000.000 Verse!</span>
                    <br><br>No transaction is required to scratch. Connect your account (<span> {{ giftAccount.slice(0,
            7) }}..</span>) to redeem the ticket.
                </p>
                <a
                    @click="closeGiftModal(true)"
                    v-if="accountActive == false"
                ><button class="btn verse-wide fixBottomMobile">Connect and Redeem</button></a>
                <a
                    @click="closeGiftModal(false)"
    ...
    >
        <div class="head">
                    ...
                    <p>
                        <label class="switch">
                            <input
                                ...
                                v-on:change="toggleFilterClaimed"
                            >
                            <span class="slider round"></span>
                        </label>Hide Claimed Tickets
                    </p>
            ...
            <div
                class="tickconnect"
                v-if="!accountActive"
            >Connect your wallet to view your tickets. </div>
        </div>
        <div class="ticket-wrapper">

            <div
                class="spin"
                v-if="accountActive && loading"
            >
                <div class="lds-ring">
                    <div></div>
                    <div></div>
            ...
            <div
                class="tickets clearfix"
                v-if="accountActive && !loading"
            >
                <div
                    v-if="nfts.length == 0"
                    class="warn-no-tickets"
                >
                ...
                >
                    ...
                    <p
                        class="status"
                        v-if="item.claimed == false && item.scratched == true"
                    >
                        Claimable Ticket
                    </p>
                    <p
                        class="status"
                        v-if="item.claimed == false && item.scratched == false"
                    >
                        Unscratched Ticket
                    </p>
                    <p
                        class="status"
                        v-if="item.claimed == true"
                    >
                        Claimed Ticket
                    </p>
                    ...
                    <div
                        v-if="item.claimed == false"
                        @click="handleClickItem(item)"
                        style="cursor: pointer"
                    >
                        <img
                            class="mobreset"
                            v-if="item.scratched == false"
                            :src="'/templates/' + item.address + '/' + item.edition + '.png'"
                        >
                        <img
                            class="mobreset unclaimed"
                            v-if="item.scratched == true"
                            :src="`https://${item.bucketUrl}.s3.amazonaws.com/${item.id}/${item.address}.jpg`"
                        >
                    </div>

                    <div v-if="item.claimed == true">
                        <img
                            class="mobreset claimed"
                            :src="`https://${item.bucketUrl}.s3.amazonaws.com/${item.id}/${item.address}.jpg`"
                        >
                    </div>

                    <button
                        v-if="item.scratched == false && item.claimed == false"
                        class="btn verse-wide secondary"
                        @click="openDetailScreen(item.id, item.address)"
                    >Scratch Ticket</button>
                    <button
                        v-if="item.scratched == true && item.claimed == false"
                        @click="openClaimDetail(item.id, item.address)"
                        class="btn verse-wide"
                    >Claim {{ item.prize }} VERSE</button>
                    <button
                        v-if="item.claimed == true"
                        class="btn verse-wide secondary disabled claimed"
                    >{{ item.prize }} VERSE Claimed</button>
                </div>
            </div>
